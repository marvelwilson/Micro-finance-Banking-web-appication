...
@Component({
  selector: 'app-rfd',
  templateUrl: './rfd.page.html',
  styleUrls: ['./rfd.page.scss'],
})
export class RfdPage implements OnInit {
...
url = 'https://agilfinance.net/service/storage/app/public/';
  ...
  async getPersonal() {
    ...
    alert.present()
    this.Httpnetwork.getPersonal('Regular Fixed').subscribe((res: any) => {
      if (res.error) {
        alert.header = 'Invalid'
        alert.message = res.error
        alert.buttons = ['OK']
  ...
  async depo() {
    ...
    if ((this.amount || this.payment != 'undefined') && (this.pro_show == 'd-block' && this.receipt != '' || this.pro_show == 'd-none' && this.receipt == '')) {
      ...
      formData.append('acc_types', 'Regular Fixed')
      formData.append('id', this.id)
  ...
  async wtrw() {
    ...
    if (this.amount != 'undefined' && this.acc_num != 'undefined') {
      ...
      formData.append('acc_types', 'Regular');
