...
@Component({
  selector: 'app-cexp',
  templateUrl: './cexp.page.html',
  styleUrls: ['./cexp.page.scss'],
})
export class CexpPage implements OnInit {
  ...
  if(data.reoccurring=='yes' && data.peroid==''){
   ...
   formdata.set('exp_amount', data.exp_amount)
   formdata.set('exp_date', data.exp_date)
   formdata.set('exp_type', data.exp_type)
   formdata.set('peroid', data.peroid)
   formdata.set('receipt', this.receipt)
   formdata.set('reoccurring', data.reoccurring)

    this.xhr.modify(formdata).subscribe((res:any)=>{
       if (res.error) {
        alert.header='Invalid'
        alert.message=res.error;
        alert.buttons=['OK']
