...
@Component({
  ...
  styleUrls: ['./recon.page.scss'],
})
export class ReconPage implements OnInit {
  items: any;
  newbank = 'd-none';
  updatebank= 'd-none'
url = 'https://agilfinance.net/service/storage/';
  ...
  async modals(params, useful) {
    ...
    if (params == 'newbank') {
     ...
     data.set('id', this.id)
     data.set ('type', 'recons')
      this.xhr.delAsset(data).subscribe((res:any)=>{
          if (res.success) {
  ...
  async getbanks() {
    ...
    alert.present();
    this.xhr.getbanks('recons').subscribe((res: any) => {
