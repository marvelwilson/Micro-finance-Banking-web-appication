...
@Component({
  selector: 'app-tempcust',
  templateUrl: './tempcust.page.html',
  styleUrls: ['./tempcust.page.scss'],
})
export class TempcustPage implements OnInit {
  items: any;
  newbank = 'd-none';
  updatebank= 'd-none'
  specific = 'd-block'
  ...
  async modals(params, useful) {
    ...
    if (params == 'newbank') {
     ...
     alert.remove()
      ...
      formdata.set('acc_type',this.acc_type)
  ...
  async getbanks() {
    ...
    alert.present();
    this.xhr.getbanks('instcusts').subscribe((res: any) => {
      if (res.error) {
        alert.header='Failed'
        alert.message=res.error,
        alert.buttons=['OK']
